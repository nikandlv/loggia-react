...
class BottomBar extends React.Component {
    render() {
        const {current, language, current_index, photos} = this.props
        return (
            <section className="bottom-bar">
                {photos.map((item,key) => {
                    return (
                        <PhotoIndicator language={this.props.language} setItem={this.props.setCurrentForce} number={key+1} key={key} light={current.bottombar_light} title={item.about[language].title} active={key === current_index} />
                    )
                })}
