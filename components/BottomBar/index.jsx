        ...
        return (
            <section className="bottom-bar">
                {this.props.items.map((item,key) => {
                    return (
                        <PhotoIndicator setItem={this.props.setItem} number={key+1} key={key} light={activeItem.light} title={item.title} active={key === this.props.active} />
                    )
                })}
                <div className={`expand-icon-wrapper ${activeItem.light?'light':''}`}>
                    <div>
